...
it("includes parallel steps", async () => {
  const mock = fetchMock
    .sandbox()
    .getOnce("https://api.github.com/repos/actions/cache/contents/action.yml?ref=v3", actionMetadata);

  const workflowWithParallelSteps = `
name: Caching Primes
...
`;
  ...
  const workflowContext = await createWorkflowContext(workflowWithParallelSteps, "build");
  const defaultContext = getDefaultStepsContext(workflowContext);
  ...
  expect(stepsContext?.get("cache-primes")).toBeDefined();
});
